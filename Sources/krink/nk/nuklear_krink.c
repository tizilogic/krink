	...
	nk_foreach(cmd, &nkctx) {
		...
		case NK_COMMAND_CURVE: {
			...
			for (u = 0; u <= 1; u += 0.0001) {
				px = pow(1 - u, 3) * c->begin.x + 3 * u * pow(1 - u, 2) * c->ctrl[0].x +
				     3 * pow(u, 2) * (1 - u) * c->ctrl[1].x + pow(u, 3) * c->end.x;
				py = pow(1 - u, 3) * c->begin.y + 3 * u * pow(1 - u, 2) * c->ctrl[0].y +
				     3 * pow(u, 2) * (1 - u) * c->ctrl[1].y + pow(u, 3) * c->end.y;
